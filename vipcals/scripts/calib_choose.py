...
def get_calib_scans(data, ordered_scan_list, refant):
    ...
    From the scan list generated by :function:`~vipcals.scripts.calib_choose.snr_scan_list_v2`, 
    choose as many scans as neccessary to cover al antennas, giving priority to the ones with 
    highest SNR. Returns a list of calibrator :class:`~vipcals.scripts.helper.Scan` along with the antennas 
    ...
    for s in scan_list:
        s.calib_antennas = \
            [x for x in s.antennas if x not in covered_antennas] + [refant]
...
def get_calib_scans_v2(data, ordered_scan_list, refant):
    """Get the scans that will be used for calibration steps.
